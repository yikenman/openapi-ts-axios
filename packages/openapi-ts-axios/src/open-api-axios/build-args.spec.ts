...
describe('buildArgs', () => {
  ...
  it('should handle methodType 2 with url and config', () => {
    const result = buildArgs(['/path', { params: { q: 1 } }], 2);
  ...
  it('should handle methodType 1 with only config', () => {
    const result = buildArgs([{ timeout: 1000 }], 1);
    expect(result).toHaveLength(1);
    expect(result[0]).toMatchObject({ timeout: 1000 });
  });
